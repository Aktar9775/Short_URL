...
function UrlList() {
  ...
  const fetchUrls = async () => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/url/all`, {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
        {filteredUrls.length > 0 ? (
          filteredUrls.map((url) => (
            <div key={url._id} className="bg-white shadow-lg rounded-xl overflow-hidden border border-gray-200 hover:shadow-xl transition duration-300">
              <div className="relative">
                <img
                  src={url.image || `https://picsum.photos/400/200?random=${Math.floor(Math.random() * 1000)}`}
                  alt="Thumbnail"
              ...
              <div className="flex justify-between items-center px-5 py-3 bg-gray-50 border-t border-gray-200">
                <button onClick={() => handleEdit(url._id)} className="flex items-center gap-2 text-blue-600 text-sm px-4 py-2 rounded-md hover:bg-blue-100 transition">
                  <FaEdit /> Edit
                </button>
                <button onClick={() => handleDelete(url._id)} className="flex items-center gap-2 text-red-500 text-sm px-4 py-2 rounded-md hover:bg-red-100 transition">
                  <FaTrash /> Delete
                </button>
              </div>
            </div>
          ...
          <p className="text-center text-gray-500 font-semibold mt-4">No URLs found. Start shortening now!</p>
        )}
      </div>
    </div>
  );
